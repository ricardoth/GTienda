...
class EliminarCategoriaScreen extends Component {
    constructor(props){
    super(props);
        this.state = {
            isLoading: true,
    ...
    cargarApiCategoria = async () =>{
        //LLama al api
        try {
            const response = await fetch('http://10.52.101.135:45455/api/Categoria/NoVigente');
    ...
    select = (item) => {
        ...
        Alert.alert(
            ...
            [
              ...
              {text: 'Si', onPress: () => this.activarCategoria(item)},
            ],
            { cancelable: false }
          )
    }
    ...
    activarCategoria = (item) => {
        ...
        //Fetch
          fetch('http://10.52.101.135:45455/api/Categoria/Activar/' + item, {
            method: 'POST',
            headers: {
          ...
          this.cargarApiCategoria();
          ToastAndroid.show('Categoría Activada!' , ToastAndroid.SHORT)
        }       
}
...
const styles = StyleSheet.create({
  ...
  export default EliminarCategoriaScreen;