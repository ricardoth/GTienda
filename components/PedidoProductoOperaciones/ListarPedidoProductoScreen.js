...
class ListarPedidoProductoScreen extends Component {
    constructor(props){
    super(props);
        this.state = {
            isLoading: true
    ...
    cargarApiPedProd(){
         //LLama al api
         return fetch('http://192.168.1.34:45455/api/PedidoProducto')
    ...
    select = (item) => {
        ...
        Alert.alert(
            ...
            [
              ...
              {text: 'Si', onPress: () => this.eliminarPedProd(item)},
            ],
            { cancelable: false }
          )
    }
    ...
    eliminarPedProd = (item) => {
        //Fetch
            fetch('http://192.168.1.34:45455/api/PedidoProducto/' + item, {
              method: 'POST',
              headers: {
            ...
            this.cargarApiPedProd();
            ToastAndroid.show('Ped Prod Eliminado ' , ToastAndroid.SHORT)
...
const styles = StyleSheet.create({
  ...
  export default ListarPedidoProductoScreen;
